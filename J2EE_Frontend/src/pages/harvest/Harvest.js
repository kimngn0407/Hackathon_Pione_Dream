...
const HarvestScreen = () => {
  ...
  const [showCropRecommendationDialog, setShowCropRecommendationDialog] = useState(false);
  const [snackbar, setSnackbar] = useState({ open: false, message: '', severity: 'success' });
  
  // Crop recommendation states
  const [envData, setEnvData] = useState({ temperature: '', humidity: '', soil_moisture: '' });
  ...
  const handleOpenCropRecommendation = () => {
    setEnvData({ temperature: '', humidity: '', soil_moisture: '' });
    setRecommendationResult(null);
    setShowCropRecommendationDialog(true);
  };
  ...
  const handleCropRecommendation = async () => {
    try {
      ...
      const requestData = {
        ...
        soil_moisture: parseFloat(envData.soil_moisture)
      };

      // Validate
      if (isNaN(requestData.temperature) || isNaN(requestData.humidity) || isNaN(requestData.soil_moisture)) {
        setSnackbar({
          open: true,
          message: 'Vui lòng nhập đầy đủ thông số môi trường!',
          severity: 'error'
        });
        return;
      }
      ...
      if (response.success) {
        ...
        setSnackbar({
          open: true,
          message: 'Đã nhận gợi ý cây trồng!',
          severity: 'success'
        });
      } else {
        setSnackbar({
          open: true,
          message: response.error || 'Có lỗi xảy ra khi gợi ý cây trồng!',
          severity: 'error'
        });
      }
    } catch (error) {
  ...
  const fillSampleNPK = () => {
    setEnvData({ temperature: '25', humidity: '80', soil_moisture: '45' });
  };
  ...
  return (
    <Container maxWidth="lg">
      <Box sx={{ mt: 4, mb: 6 }}>
              ...
              <Grid container spacing={2} alignItems="center">
                ...
                <Grid item xs={12} md={2}>
                  <Button
                    fullWidth
                    variant="contained"
                    onClick={handleOpenCropRecommendation}
        ...
        <Dialog
          open={showCropRecommendationDialog}
          onClose={() => setShowCropRecommendationDialog(false)}
          maxWidth="sm"
          fullWidth
          PaperProps={{
            sx: {
        ...
        >
          ...
          >
            ...
            <DialogTitle sx={{ p: 0, color: '#9a3412', fontWeight: 700, fontSize: '1.5rem' }}>
              🌱 Gợi Ý Cây Trồng Cho Mùa Tiếp Theo
            </DialogTitle>
            <Typography variant="body2" sx={{ color: '#9a3412', mt: 1, opacity: 0.8 }}>
              Nhập thông số môi trường (Nhiệt độ, Độ ẩm, Độ ẩm đất) để nhận gợi ý cây trồng phù hợp
            </Typography>
          </Box>
          
          <DialogContent sx={{ p: 4 }}>
            <Box sx={{ mb: 3 }}>
              <Alert severity="info" icon={<LightbulbIcon />} sx={{ borderRadius: '12px', mb: 3 }}>
                <strong>Tip:</strong> Nhập thông số môi trường (Nhiệt độ, Độ ẩm không khí, Độ ẩm đất).
                Bạn có thể lấy từ cảm biến hoặc dùng dữ liệu mẫu.
              </Alert>
              ...
              <Grid container spacing={2}>
                <Grid item xs={4}>
                  <TextField
                    ...
                    InputProps={{
                      endAdornment: <Typography variant="caption" color="text.secondary">°C</Typography>
                    }}
                    sx={{
                      '& .MuiOutlinedInput-root': {
                        borderRadius: '12px',
                ...
                <Grid item xs={4}>
                  <TextField
                    ...
                    label="Độ ẩm đất"
                    type="number"
                    value={envData.soil_moisture}
                    onChange={(e) => setEnvData({ ...envData, soil_moisture: e.target.value })}
                    InputProps={{
                      endAdornment: <Typography variant="caption" color="text.secondary">%</Typography>
                    }}
                    sx={{
            ...
            {recommendationResult && (
              ...
              >
                ...
                >
                  <LocalFloristIcon sx={{ fontSize: 30 }} />
                </Avatar>
                <Typography variant="h5" sx={{ fontWeight: 700, color: '#166534', mb: 1 }}>
                  {recommendationResult.recommended_crop}
                </Typography>
                {recommendationResult.crop_name_en && (
                  <Typography variant="body2" sx={{ color: '#15803d', mb: 2, fontStyle: 'italic' }}>
                    ({recommendationResult.crop_name_en})
                  </Typography>
                )}
                {recommendationResult.confidence && (
                  <Box sx={{ mt: 2 }}>
                    <Typography variant="body2" sx={{ mb: 1, color: '#166534', fontWeight: 600 }}>
                      Độ tin cậy: {(recommendationResult.confidence * 100).toFixed(1)}%
                    </Typography>
                    <Box
                      sx={{
                        height: 8,
                    ...
                    >
                      <Box
                        sx={{
                          height: '100%',
                          width: `${recommendationResult.confidence * 100}%`,
          ...
          <DialogActions sx={{ p: 3, gap: 2, borderTop: '1px solid #e0e0e0' }}>
            <Button
              onClick={() => setShowCropRecommendationDialog(false)}
              variant="outlined"
              sx={{
                borderRadius: '12px',
                px: 3,
            ...
            <Button
              ...
              disabled={recommendationLoading || !envData.temperature || !envData.humidity || !envData.soil_moisture}
              variant="contained"
              startIcon={recommendationLoading ? <CircularProgress size={20} color="inherit" /> : <LightbulbIcon />}
              sx={{
                borderRadius: '12px',
                px: 3,
            ...
            >
              {recommendationLoading ? 'Đang phân tích...' : 'Nhận gợi ý'}
            </Button>
          </DialogActions>
        </Dialog>
